...
class Graphic:
    ...
    def rect_full(self, color, x, y, width, height, radius):
        button = pygame.draw.rect(
            self.Window, color, pygame.Rect(x, y, width, height), 0, radius
        )
        return button
    ...
    def button_hover(
        ...
        if self.is_mouse_over_button(name):
            self.rect_border(
                color_border_hover, x, y, width + 5, height + 5, thickness, radius
            )
        else:
    ...
    def element(self):
        ...
        self.button_solve = self.button_hover(
            "resolution",
            400,
            500,
            150,
    ...
    def draw_disks(self):
        ...
        for i, disk in enumerate(self.t_1):
            self.rect_full_border(
                colors[disk - 1], 100, 387 - i * 15, 150 - (8 - disk) * 10, 15, 0, 5
            )

        # Dessiner les disques sur la Tour 2
        for i, disk in enumerate(self.t_2):
            self.rect_full_border(
                colors[disk - 1], 400, 387 - i * 15, 150 - (8 - disk) * 10, 15, 0, 5
            )

        # Dessiner les disques sur la Tour 3
        for i, disk in enumerate(self.t_3):
            self.rect_full_border(
                colors[disk - 1], 700, 387 - i * 15, 150 - (8 - disk) * 10, 15, 0, 5
            )
